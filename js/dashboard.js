...
let memberSince = new Date(user(currentUser.name).createdAt).toLocaleDateString('en-US', { month: 'long', year: 'numeric' });


localStorage.setItem('users', JSON.stringify(User));


const initials = user(currentUser.name).fullName.split(' ').map(n => n[0]).join('').toUpperCase();
...
function updateUserStats() {
    ...
    // document.getElementById("currentUser").innerHTML = currentUser ? currentUser.name : "Guest"

    // Update user profile picture
    // userProfile.innerHTML = `
    //     <div class="w-12 sm:w-16 h-12 sm:h-16 rounded-full flex items-center justify-center text-white text-xl sm:text-2xl font-bold bg-purple-500">
    //         <span>${initials}</span>
    //     </div>`
